...
class Tasks():
    ...
    def FindMainWindow(img):
        ...
        contours, hierarchy = Contours.GetContours(dilate)
        orig_contour = ImageLoaders.Deserialize("maintests")
        orig_contour_length = Contours.GetContourLength(orig_contour)

        for contour in contours:
            temp_contour_length = Contours.GetContourLength(contour)
            ...
            and orig_contour_length < temp_contour_length + CommonMethods.GetPercent(temp_contour_length, 30)):
                match = Contours.GetMatchShapes(orig_contour, contour)
                if match < 0.07:
                    Contours.DrawRectangle(contour, img)
    ...
    def FindFilter(img):
        ...
        contours, hierarchy = Contours.GetContours(th)
